...
class Event:
    ...
    def getEventName(self):
        ...
        self.Log.logContent(str(self.event_id) + ": Event.getEventName complete")
        return self.name
    ...
    def addQueue(self,name,queue_id):
        self.Log.logContent(str(self.event_id) + ": Event.addQueue({0},{1})".format(name,queue_id))
        if(self.isLocked == False):
            self.all_queues[queue_id]=GQueue(name,queue_id)
            self.Log.logContent(str(self.event_id) + ": Event.addQueue({0},{1}) complete".format(name,queue_id))
            return True
        else:
            self.Log.logContent(str(self.event_id) + ": Event.addQueue({0},{1}) error".format(name,queue_id))
            return False
    
    # delete a specific queue in the event
    # arguments: id of that queue
    def deleteQueue(self,queue_id):
        self.Log.logContent(str(self.event_id) + ": Event.deleteQueue({0})".format(queue_id))
        if(self.isLocked == False):
            self.all_queues.pop(queue_id)
            self.Log.logContent(str(self.event_id) + ": Event.deleteQueue({0}) complete".format(queue_id))
            return True
        else:
            self.Log.logContent(str(self.event_id) + ": Event.deleteQueue({0}) error".format(queue_id))
            return False
