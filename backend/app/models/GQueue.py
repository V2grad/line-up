...
class Queue:
    ...
    def removeUser(self,user):
        self.Log.logcontent(self.queue_id+": Queue.removeUser({0})".format(user.user_id))
        if(not self.isLock):
            self.queue.remove(user)
            self.Log.logcontent(self.queue_id+": Queue.removeUser({0}) complete".format(user.user_id))
            return True
        else:
            self.Log.logcontent(self.queue_id+": Queue.removeUser({0}) error".format(user.user_id))
            return False
    
    #remove a user in the queue by using user ID
    #argument: id of that user
    def removeUserById(self,id):
        self.Log.logcontent(self.queue_id+": Queue.removeUser({0})".format(id))
        if(not self.isLock):
            for i in self.queue:
                if id == i.user_id:
                    self.Log.logcontent(self.queue_id+": Queue.removeUser({0}) complete".format(id))
                    return True
            self.Log.logcontent(self.queue_id+": Queue.removeUser({0}) error ID not found".format(id))
            return False
        else:
